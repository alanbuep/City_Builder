...
export class DisasterModal {
  private modal = new Modal('Catástrofes', 'alert');
  ...
  constructor(cb: DisasterModalCallbacks) {
    ...
    intro.textContent =
      ...
      'Los edificios dañados quedan en ruinas hasta repararlos. Sobrevivir a una catástrofe con daño da una ficha de territorio (con un tiempo de espera entre premios).';
    ...
    add('flame', 'Incendio', 'Se prende fuego un edificio al azar. Los bomberos cercanos (o el héroe) lo apagan.', cb.onTriggerFire);
    add('sparkles', 'Meteorito', 'Cae sobre la ciudad: arrasa e incendia la zona del impacto.', cb.onTriggerMeteor);
    add('wind', 'Tornado', 'Cruza el mapa serpenteando y daña lo que toca.', cb.onTriggerTornado);
    add('refresh', 'Huracán', 'Castiga TODA la ciudad. El más destructivo.', cb.onTriggerHurricane);
  ...
  private setRandomLabel(): void {
    this.randomBtn.innerHTML = `${icon('refresh', 16)}<span>Catástrofes al azar: ${this.random ? 'ON' : 'OFF'}</span>`;
  }
