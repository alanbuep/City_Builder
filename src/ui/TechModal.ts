...
export class TechModal {
  private modal = new Modal('Tecnología', 'flask');
  private sig = '';
  ...
  update(status: TechStatus, science: { total: number; rate: number }): void {
    ...
    html += `<div class="stat"><span class="bi-name">${icon('flask', 16)}Ciencia acumulada</span><span class="val">${science.total.toLocaleString('es')}${
      science.rate > 0 ? ` <span style="font-size:11px; color:#26c6da">(+${science.rate}/mes)</span>` : ''
    }</span></div>`;
    html += `<div class="stat" style="margin-bottom:8px"><span class="bi-name">${icon('star', 16)}Desbloqueos</span><span class="val">${status.unlocked} / ${status.total}</span></div>`;
    if (!n) {
      html += '<b style="color:#7CFC9A">¡Todo desbloqueado!</b>';
    } else {
      const fmt = (v: number) => (n.isMoney ? `$${v.toLocaleString('es')}` : v.toLocaleString('es'));
      html +=
        `<div class="tech-next">Próximo: <b>${n.name}</b><br>` +
